...
export default function ValueDepreciationCalculator() {
  ...
  const calculate = () => {
    ...
    setDifference(futureValue - amount);

    const data = [];
    for (let i = 0; i <= years[0]; i++) {
      const value = amount / Math.pow(1 + rate[0] / 100, i);
      data.push({ year: i, value: parseFloat(value.toFixed(2)) });
    }
    setChartData(data);
  };
            ...
            <div className="bg-slate-200 p-8 rounded-2xl space-y-6 w-full">
              <div className="space-y-3">
                ...
                <div className="grid sm:grid-cols-2 gap-4 text-gray-700 dark:text-gray-300 text-base font-medium">
                  <p><span className="font-semibold">Current Value:</span> ₹{amount}</p>
                  <p><span className="font-semibold">Future Value:</span> ₹{result.toFixed(2)}</p>
